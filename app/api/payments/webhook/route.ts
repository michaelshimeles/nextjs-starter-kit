    ...
    switch (event.type) {
      ...
      case "customer.subscription.updated": {
        ...
        if (customerEmail) {
          ...
          const { data, error } = await supabase
            .from("subscriptions")
            [operation](
              operation === "insert" ? [subscriptionData] : subscriptionData
            )
          ...
          if (error) {
            ...
            return NextResponse.json({
              status: 500,
              error: `Error during subscription ${operation}`,
            });
          }

          return NextResponse.json({
            status: 200,
            message: `Subscription ${operation} success`,
            data,
          });
        }
      ...
      case "customer.subscription.deleted": {
        ...
        if (customerEmail) {
          ...
          if (error) {
            ...
            return NextResponse.json({
              status: 500,
              error: "Error updating subscription (cancelled)",
            });
          }
      ...
      case "invoice.payment_succeeded": {
        ...
        if (customerEmail) {
          ...
          if (error) {
            console.error(
              "Error inserting invoice (payment succeeded):",
              error
            );
            return NextResponse.json({
              status: 500,
              error: "Error inserting invoice (payment succeeded)",
            });
          }
      ...
      case "invoice.payment_failed": {
        ...
        if (customerEmail) {
          ...
          if (error) {
            ...
            return NextResponse.json({
              status: 500,
              error: "Error inserting invoice (payment failed)",
            });
          }
